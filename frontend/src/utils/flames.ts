...
// ── Valeurs par défaut (modifiables ici ou via le DebugPanel) ──────────────────
...
const FIRE_COLORS = [
  ...
  "#ff8800", "#ffaa00", "#ffcc00", "#ffee44",
];

// ── Utilitaire DOM ─────────────────────────────────────────────────────────────

function mkFlameEl(x: number, y: number, color: string, w: number, h: number): HTMLDivElement {
...
export function spawnLoseFlames(
  ...
  function spawnOne(spawnX: number, isCore: boolean, waveDelay: number) {
    ...
    const el = mkFlameEl(spawnX, bBottom, FIRE_COLORS[colorIndex], size, size * (1.3 + Math.random() * 0.8));
    ...
    const riseDur    = C.riseDurMin + Math.random() * (C.riseDurMax - C.riseDurMin);
  ...
  for (let wave = 0; wave < C.waveCount; wave++) {
    ...
    for (let i = 0; i < perWave; i++) {
      spawnOne(bLeft + Math.random() * bWidth, Math.random() < C.coreRatio, waveDelay);
    }
  }
}
